...
class Settings(BaseModel):
    ...
    tool_policy: str = Field(default_factory=lambda: os.getenv("TOOL_POLICY", "allow"))  # allow, sanitize, block_all
    ...
    strict_isolation: bool = Field(
        default_factory=lambda: os.getenv("STRICT_ISOLATION", "false").lower() in ("true", "1", "yes")
    )
